...
Aen_unrelated_permutation <- read.csv(here("Data/Aen_Ham", "Aen_unrelated_permutation_div.csv"))
...
#mean Ho & He for each population
Aen_Ho_means <- colMeans(sum_stats_Aen$Ho) #1: 0.06302, 2: 0.09171
Aen_Hs_means <- colMeans(sum_stats_Aen$Hs) #1: 0.11077, 2: 0.09179

#Ho % change: 45.52% increase
#He % change: 17.13% decrease

Aen_Ho_means_diff <- diff(colMeans(sum_stats_Aen$Ho)) #0.02869
Aen_Hs_means_diff <- diff(colMeans(sum_stats_Aen$Hs)) #-0.01899
...
#calculate mean Fis
Hist_Aen_mean_Fis <- mean(Hist_Aen_div$Fis) #0.15783
Contemp_Aen_mean_Fis <- mean(Contemp_Aen_div$Fis) #0.00112

Contemp_Aen_mean_Fis - Hist_Aen_mean_Fis #-0.15671
...
## write out Ho, He, Fis summary tables ##
Ho_mean <- as.data.frame(c(0.06302, 0.09171))
He_mean <- as.data.frame(c(0.11077, 0.09179))
Fis_mean <- as.data.frame(c(0.15783, 0.00112))
...
Aen_Ho_boot_diff$Ho_diff <- Aen_Ho_boot_diff$Contemp_Ho - Aen_Ho_boot_diff$Hist_Ho
  Aen_Ho_boot_diff_CIs <- quantile(Aen_Ho_boot_diff$Ho_diff, c(0.025, 0.975)) #0.02245, 0.03543
...
Aen_He_boot_diff$He_diff <- Aen_He_boot_diff$Contemp_He - Aen_He_boot_diff$Hist_He
  Aen_He_boot_diff_CIs <- quantile(Aen_He_boot_diff$He_diff, c(0.025, 0.975)) #-0.02674, -0.01133
...
Aen_Fis_boot_diff$Fis_diff <- Aen_Fis_boot_diff$Contemp_Fis - Aen_Fis_boot_diff$Hist_Fis
  Aen_Fis_boot_diff_CIs <- quantile(Aen_Fis_boot_diff$Fis_diff, c(0.025, 0.975)) #-0.16768, -0.14598

## Calculate empirical p-value ##
#Ho
Ho_perm_greater <- as.numeric(nrow(subset(Aen_permutation, Ho > 0.02869))) #number of permutations with value greater than observed
  (Ho_perm_greater + 1)/10001 #0.0001
  
#He
He_perm_greater <- as.numeric(nrow(subset(Aen_permutation, He < -0.01899)))
  (He_perm_greater + 1)/10001 #0.0001
  
#Fis
Fis_perm_greater <- as.numeric(nrow(subset(Aen_permutation, Fis < -0.15671)))
...
Ho_null_plot <- ggplot(Aen_permutation, aes(x = Ho)) + 
  ...
  geom_vline(aes(xintercept = 0), linewidth = 4, color = "black") +
  geom_vline(aes(xintercept = -0.00480), linewidth = 6, linetype = "dashed", color = "#bd8850") + #0.25 CI lower
  geom_vline(aes(xintercept = 0.00519), linewidth = 6, linetype = "dashed", color = "#bd8850") + #0.95 CI upper
  geom_vline(aes(xintercept = 0.02869), linewidth = 6, color = "#a25505") + #"real" mean diff
...
He_null_plot <- ggplot(Aen_permutation, aes(x = He)) + 
  geom_density(color = "#e3ccb4", fill = "#e3ccb4") + 
  geom_vline(aes(xintercept = 0), linewidth = 4, color = "black") +
  geom_vline(aes(xintercept = -0.00456), linewidth = 6, linetype = "dashed", color = "#bd8850") + #0.25 CI lower
  geom_vline(aes(xintercept = 0.00409), linewidth = 6, linetype = "dashed", color = "#bd8850") + #0.95 CI upper
  geom_vline(aes(xintercept = -0.01899), linewidth = 6, color = "#a25505") + #"real" mean diff
...
Fis_null_plot <- ggplot(Aen_permutation, aes(x = Fis)) + 
  geom_density(color = "#e3ccb4", fill = "#e3ccb4") + 
  geom_vline(aes(xintercept = 0), linewidth = 4, color = "black") +
  geom_vline(aes(xintercept = -0.04700), linewidth = 6, linetype = "dashed", color = "#bd8850") + #0.25 CI lower
  geom_vline(aes(xintercept = 0.03421), linewidth = 6, linetype = "dashed", color = "#bd8850") + #0.95 CI upper
  geom_vline(aes(xintercept = -0.15671), linewidth = 6, color = "#a25505") + #"real" mean diff
...
######## Aen unrelated Ho, He & Fis estimates ########

#add population level data
pop <- c(rep(1, times = 30), rep(2, times = 84))
  pop(Aen_unrelated_genind) <- pop #1 = Albatross, 2 = Contemporary
  Aen_unrelated_genind #check to make sure pops read right (should range from 55-92 individuals)

#calculate diversity metrics w/in pops
sum_stats_Aen_unrelated <- basic.stats(Aen_unrelated_genind)

#### Calculate mean Ho, He ####

#mean Ho & He for each population
Aen_unrelated_Ho_means <- colMeans(sum_stats_Aen_unrelated$Ho) #1: 0.06302, 2: 0.09194
Aen_unrelated_Hs_means <- colMeans(sum_stats_Aen_unrelated$Hs) #1: 0.11077, 2: 0.09195

Aen_unrelated_Ho_means_diff <- diff(colMeans(sum_stats_Aen_unrelated$Ho)) #0.02892
Aen_unrelated_Hs_means_diff <- diff(colMeans(sum_stats_Aen_unrelated$Hs)) #-0.01882

##### Pull out per-marker estimates ####

#pull out Ho for each population
Hist_Aen_unrelated_Ho <- sum_stats_Aen_unrelated$Ho[,1]
Contemp_Aen_unrelated_Ho <- sum_stats_Aen_unrelated$Ho[,2]

#pull out He for each population
Hist_Aen_unrelated_He <- sum_stats_Aen_unrelated$Hs[,1]
Contemp_Aen_unrelated_He <- sum_stats_Aen_unrelated$Hs[,2]

#bind Ho & He into dataframe for each pop
Hist_Aen_unrelated_div <- as.data.frame(cbind(Hist_Aen_unrelated_Ho, Hist_Aen_unrelated_He))
  head(Hist_Aen_unrelated_div) #check content is right
  colnames(Hist_Aen_unrelated_div) <- c("Ho", "He")

Contemp_Aen_unrelated_div <- as.data.frame(cbind(Contemp_Aen_unrelated_Ho, Contemp_Aen_unrelated_He))
  colnames(Contemp_Aen_unrelated_div) <- c("Ho", "He")

##### Calculate mean Fis ####

#calculate Fis for each population
Hist_Aen_unrelated_div$Fis <- 1 - (Hist_Aen_unrelated_div$Ho/Hist_Aen_unrelated_div$He)
  Hist_Aen_unrelated_div$Fis[is.nan(Hist_Aen_unrelated_div$Fis)] <- 0

Contemp_Aen_unrelated_div$Fis <- 1 - (Contemp_Aen_unrelated_div$Ho/Contemp_Aen_unrelated_div$He)
  Contemp_Aen_unrelated_div$Fis[is.nan(Contemp_Aen_unrelated_div$Fis)] <- 0

#calculate mean Fis
Hist_Aen_unrelated_mean_Fis <- mean(Hist_Aen_unrelated_div$Fis) #0.15783
Contemp_Aen_unrelated_mean_Fis <- mean(Contemp_Aen_unrelated_div$Fis) #0.00112

Contemp_Aen_unrelated_mean_Fis - Hist_Aen_unrelated_mean_Fis #-0.15671

#write out
#write.csv(Hist_Aen_unrelated_div, "Data/Aen_Ham/Hist_Aen_unrelated_div.csv", quote = FALSE, row.names = TRUE)
#write.csv(Contemp_Aen_unrelated_div, "Data/Aen_Ham/Contemp_Aen_unrelated_div.csv", quote = FALSE, row.names = TRUE)

#### Ho & He bootstrapping ####

#read in if running separately
#Hist_Aen_unrelated_div <- read.csv("Data/Aen_Ham/Hist_Aen_unrelated_div.csv")
#colnames(Hist_Aen_unrelated_div) <- c("locus", "Ho", "He", "Fis")
#Contemp_Aen_unrelated_div <- read.csv("Data/Aen_Ham/Contemp_Aen_unrelated_div.csv")
#colnames(Contemp_Aen_unrelated_div) <- c("locus", "Ho", "He", "Fis")

## Ho bootstrapping ##
#historical Ho
boot_Hist_Aen_unrelated_Ho <- boot(data = Hist_Aen_unrelated_div$Ho, 
                                   statistic = samp_mean, R = 1000) #1000 permutations of Ho
  Hist_Aen_unrelated_Ho_95ci <- boot.ci(boot_Hist_Aen_unrelated_Ho, conf = 0.95, 
                                        type = "norm") #get 95% CI for Ho
  Hist_Aen_unrelated_Ho_95ci_normal <- Hist_Aen_unrelated_Ho_95ci$normal #pull out normal distribution 2.5 & 97.5 percentiles for Ho

#contemporary Ho
boot_Contemp_Aen_unrelated_Ho <- boot(data = Contemp_Aen_unrelated_div$Ho, 
                                      statistic = samp_mean, R = 1000)
  Contemp_Aen_unrelated_Ho_95ci <- boot.ci(boot_Contemp_Aen_unrelated_Ho, conf = 0.95, 
                                           type = "norm")
  Contemp_Aen_unrelated_Ho_95ci_normal <- Contemp_Aen_unrelated_Ho_95ci$normal

## He bootstrapping ##
#historical He
boot_Hist_Aen_unrelated_He <- boot(data = Hist_Aen_unrelated_div$He, 
                                   statistic = samp_mean, R = 1000)
  Hist_Aen_unrelated_He_95ci <- boot.ci(boot_Hist_Aen_unrelated_He, conf = 0.95, 
                                        type = "norm")
  Hist_Aen_unrelated_He_95ci_normal <- Hist_Aen_unrelated_He_95ci$normal

#contemporary He
boot_Contemp_Aen_unrelated_He <- boot(data = Contemp_Aen_unrelated_div$He, 
                                      statistic = samp_mean, R = 1000)
Contemp_Aen_unrelated_He_95ci <- boot.ci(boot_Contemp_Aen_unrelated_He, conf = 0.95, 
                                         type = "norm")
Contemp_Aen_unrelated_He_95ci_normal <- Contemp_Aen_unrelated_He_95ci$normal

## Fis bootstrapping ##
#historical Fis
boot_Hist_Aen_unrelated_Fis <- boot(data = Hist_Aen_unrelated_div$Fis, 
                                    statistic = samp_mean, R = 1000)
Hist_Aen_unrelated_Fis_95ci <- boot.ci(boot_Hist_Aen_unrelated_Fis, conf = 0.95, 
                                       type = "norm")
Hist_Aen_unrelated_Fis_95ci_normal <- Hist_Aen_unrelated_Fis_95ci$normal

#contemporary Fis
boot_Contemp_unrelated_Aen_Fis <- boot(data = Contemp_Aen_unrelated_div$Fis, 
                                       statistic = samp_mean, R = 1000)
Contemp_Aen_unrelated_Fis_95ci <- boot.ci(boot_Contemp_unrelated_Aen_Fis, conf = 0.95, 
                                          type = "norm")
Contemp_Aen_unrelated_Fis_95ci_normal <- Contemp_Aen_unrelated_Fis_95ci$normal

## write out Ho, He, Fis summary tables ##
Ho_mean <- as.data.frame(c(0.06302, 0.09194))
He_mean <- as.data.frame(c(0.11077, 0.09195))
Fis_mean <- as.data.frame(c(0.15783, 0.00112))

#Ho summary table
Ho_ci <- rbind(Hist_Aen_unrelated_Ho_95ci_normal, Contemp_Aen_unrelated_Ho_95ci_normal) #combine df w/ci info for each pop into one dataframe
Ho_sum <- cbind(Ho_mean, Ho_ci)
  colnames(Ho_sum) <- c("mean", "ci", "2.5_per", "97.5_per")
  Ho_sum$Pop <- c("Hist", "Contemp")
...
#He summary table
He_ci <- rbind(Hist_Aen_unrelated_He_95ci_normal, Contemp_Aen_unrelated_He_95ci_normal)
He_sum <- cbind(He_mean, He_ci)
  colnames(He_sum) <- c("mean", "ci", "2.5_per", "97.5_per")
  He_sum$Pop <- c("Hist", "Contemp")
...
#Fis summary table
Fis_ci <- rbind(Hist_Aen_unrelated_Fis_95ci_normal, Contemp_Aen_unrelated_Fis_95ci_normal)
Fis_sum <- cbind(Fis_mean, Fis_ci)
  colnames(Fis_sum) <- c("mean", "ci", "2.5_per", "97.5_per")
  Fis_sum$Pop <- c("Hist", "Contemp")
...
#write out data
#write.csv(div_sum_all, "Data/Aen_Ham/Aen_unrelated_div_cis.csv")

## Calculate 95% for diff through time ##
#Ho diff
Aen_unrelated_Ho_boot_diff <- as.data.frame(boot_Hist_Aen_unrelated_Ho$t)
  Aen_unrelated_Ho_boot_diff$Contemp_Ho <- boot_Contemp_Aen_unrelated_Ho$t
  colnames(Aen_unrelated_Ho_boot_diff) <- c("Hist_Ho", "Contemp_Ho")

Aen_unrelated_Ho_boot_diff$Ho_diff <- Aen_unrelated_Ho_boot_diff$Contemp_Ho - Aen_unrelated_Ho_boot_diff$Hist_Ho
Aen_unrelated_Ho_boot_diff_CIs <- quantile(Aen_unrelated_Ho_boot_diff$Ho_diff, 
                                           c(0.025, 0.975)) #0.02219, 0.03570

#He diff
Aen_unrelated_He_boot_diff <- as.data.frame(boot_Hist_Aen_unrelated_He$t)
  Aen_unrelated_He_boot_diff$Contemp_He <- boot_Contemp_Aen_unrelated_He$t
  colnames(Aen_unrelated_He_boot_diff) <- c("Hist_He", "Contemp_He")

Aen_unrelated_He_boot_diff$He_diff <- Aen_unrelated_He_boot_diff$Contemp_He - Aen_unrelated_He_boot_diff$Hist_He
Aen_unrelated_He_boot_diff_CIs <- quantile(Aen_unrelated_He_boot_diff$He_diff, 
                                           c(0.025, 0.975)) #-0.02638, -0.01149

#Fis diff
Aen_unrelated_Fis_boot_diff <- as.data.frame(boot_Hist_Aen_unrelated_Fis$t)
Aen_unrelated_Fis_boot_diff$Contemp_Fis <- boot_Contemp_unrelated_Aen_Fis$t
  colnames(Aen_unrelated_Fis_boot_diff) <- c("Hist_Fis", "Contemp_Fis")

Aen_unrelated_Fis_boot_diff$Fis_diff <- Aen_unrelated_Fis_boot_diff$Contemp_Fis - Aen_unrelated_Fis_boot_diff$Hist_Fis
Aen_unrelated_Fis_boot_diff_CIs <- quantile(Aen_unrelated_Fis_boot_diff$Fis_diff, 
                                            c(0.025, 0.975)) #-0.16767, -0.14648

## Calculate empirical p-value ##
#Ho
Ho_perm_greater <- as.numeric(nrow(subset(Aen_unrelated_permutation, Ho > 0.02892))) #number of permutations with value greater than observed
(Ho_perm_greater + 1)/10001 #0.0001

#He
He_perm_greater <- as.numeric(nrow(subset(Aen_unrelated_permutation, He < -0.01882)))
(He_perm_greater + 1)/10001 #0.0001

#Fis
Fis_perm_greater <- as.numeric(nrow(subset(Aen_unrelated_permutation, Fis < -0.15671)))
(Fis_perm_greater + 1)/10001 #0.0001

#### Aen Ho, He & Fis visualization ####

#read in data (if running separately)
#div_sum_all <- read.csv("PIRE_Aen_Ham/Aen_unrelated_div_cis.csv", row.names = 1)
...
Het_plot <- ggplot(data = het_sum_all, aes(x = metric, y = mean, color = Pop, shape = Pop)) + 
  ...
  scale_color_manual(values = c("#a25505", "#e3ccb4")) +
  scale_shape_manual(values = c(19, 15)) +
  ylim(0, 0.15) + ggtitle("Aen unrelated Ho & He 95% CIs") + theme_bw() + 
  theme(panel.border = element_rect(linewidth = 1), 
        plot.title = element_text(size = 30, color = "black", face = "bold"),
        axis.title = element_text(size = 28, color = "black"), 
        axis.ticks = element_line(color = "black", linewidth = 1), 
...
#plot of null Ho diff distribution
Ho_null_plot <- ggplot(Aen_unrelated_permutation, aes(x = Ho)) + 
  geom_density(color = "#e3ccb4", fill = "#e3ccb4") + 
  geom_vline(aes(xintercept = 0), linewidth = 4, color = "black") +
  geom_vline(aes(xintercept = -0.00515), linewidth = 6, linetype = "dashed", color = "#bd8850") + #0.25 CI lower
  geom_vline(aes(xintercept = 0.00555), linewidth = 6, linetype = "dashed", color = "#bd8850") + #0.95 CI upper
  geom_vline(aes(xintercept = 0.02892), linewidth = 6, color = "#a25505") + #"real" mean diff
  annotate("text", x = -0.013, y = 242, label = "D", size = 40) + 
  scale_y_continuous(expand = c(0, 0)) + 
  scale_x_continuous(labels = scales::label_number()) + 
  xlab(bquote("Temporal Change in"~H[o])) + ylab("Density") + 
...
#plot of null He diff distribution
He_null_plot <- ggplot(Aen_unrelated_permutation, aes(x = He)) + 
  geom_density(color = "#e3ccb4", fill = "#e3ccb4") + 
  geom_vline(aes(xintercept = 0), linewidth = 4, color = "black") +
  geom_vline(aes(xintercept = -0.00474), linewidth = 6, linetype = "dashed", color = "#bd8850") + #0.25 CI lower
  geom_vline(aes(xintercept = 0.00417), linewidth = 6, linetype = "dashed", color = "#bd8850") + #0.95 CI upper
  geom_vline(aes(xintercept = -0.01882), linewidth = 6, color = "#a25505") + #"real" mean diff
  annotate("text", x = -0.0295, y = 185, label = "A", size = 40) + 
  scale_y_continuous(expand = c(0, 0)) + 
  scale_x_continuous(labels = scales::label_number()) + 
  xlab(bquote("Temporal Change in"~H[e])) + ylab("Density") + 
...
#plot of null Fis diff distribution
Fis_null_plot <- ggplot(Aen_unrelated_permutation, aes(x = Fis)) + 
  geom_density(color = "#e3ccb4", fill = "#e3ccb4") + 
  geom_vline(aes(xintercept = 0), linewidth = 4, color = "black") +
  geom_vline(aes(xintercept = -0.04712), linewidth = 6, linetype = "dashed", color = "#bd8850") + #0.25 CI lower
  geom_vline(aes(xintercept = 0.03500), linewidth = 6, linetype = "dashed", color = "#bd8850") + #0.95 CI upper
  geom_vline(aes(xintercept = -0.15671), linewidth = 6, color = "#a25505") + #"real" mean diff
  annotate("text", x = -0.1155, y = 13.5, label = "J", size = 40) + 
  scale_y_continuous(expand = c(0, 0)) + 
  scale_x_continuous(labels = scales::label_number()) + 
...
#read in data
Aen_Fis_PCA <- read.csv(here("Data/Aen_Ham", "Fis_PCAloadings.csv"))
Aen_unrelated_Fis_PCA <- read.csv(here("Data/Aen_Ham", "Fis_PCAloadings_unrelated.csv"))

#### Aen Fis v PCA plots ####
histFis_PCA1_scatter_plot <- ggplot(data = Aen_Fis_PCA, aes(x = Historical_Fis, y = PC_1)) + 
  geom_point(size = 16, color = "#a25505", alpha = 0.8) + 
  geom_smooth(method = "lm", color = "black",linewidth = 2) +
  labs(y = "PC 1 loadings", x = "Historical Fis") + 
  theme_bw() + 
  theme(panel.border = element_rect(linewidth = 4), 
        ...
        axis.title.x = element_text(size = 55, color = "black", vjust = -1), 
        plot.margin = unit(c(1,1,1,1), "cm"),)
histFis_PCA1_scatter_plot

ContempFis_PCA1_scatter_plot <- ggplot(data = Aen_Fis_PCA, aes(x = Contemp_Fis, y = PC_1)) + 
  geom_point(size = 16, color = "#e3ccb4", alpha = 0.8) + 
  geom_smooth(method = "lm", color = "black",linewidth = 2) +
  labs(y = "PC 1 loadings", x = "Contemporary Fis") + 
  theme_bw() + 
  theme(panel.border = element_rect(linewidth = 4), 
        plot.title = element_blank(),
        axis.ticks = element_line(color = "black", linewidth = 4), 
        ...
        axis.title.x = element_text(size = 55, color = "black", vjust = -1), 
        plot.margin = unit(c(1,1,1,1), "cm"),)
ContempFis_PCA1_scatter_plot

#### Aen unrelated Fis v PCA plots ####
histFis_PCA1_scatter_plot <- ggplot(data = Aen_unrelated_Fis_PCA, aes(x = Hist_Fis, y = PC_1)) + 
  geom_point(size = 16, color = "#a25505", alpha = 0.8) + 
  geom_smooth(method = "lm", color = "black",linewidth = 2) +
  labs(y = "PC 1 loadings", x = "Historical Fis") + 
  theme_bw() + 
...
ContempFis_PCA1_scatter_plot <- ggplot(data = Aen_unrelated_Fis_PCA, aes(x = Contemp_Fis, y = PC_1)) + 
  geom_point(size = 16, color = "#e3ccb4", alpha = 0.8) + 
  geom_smooth(method = "lm", color = "black",linewidth = 2) +
  labs(y = "PC 1 loadings", x = "Contemporary Fis") + 
  theme_bw() + 

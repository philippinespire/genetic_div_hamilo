...
Aen_pi <- read.table(here("Data/Aen_Ham/pixy", "pixy_pi.txt"), header = TRUE)
  Aen_unrelated_pi <- read.table(here("Data/Aen_Ham/pixy", "pixy_unrelated_pi.txt"), header = TRUE)
...
Aen_pi_permutation <- read.csv(here("Data/Aen_Ham", "Aen_permutation_pi.csv"))
  Aen_unrelated_pi_permutation <- read.csv(here("Data/Aen_Ham", "Aen_unrelated_permutation_pi.csv"))
...
#subsetting dataframe to windows with >100 genotyped sites
Aen_pi_100bp <- subset(Aen_pi, no_sites >= 100) #left with 3029 windows (~1514/time point)

#create dataframe for each pop
Aen_AHam_pi_100bp <- subset(Aen_pi_100bp, pop == "AHam") #1514 windows
Aen_CHam_pi_100bp <- subset(Aen_pi_100bp, pop == "CBat") #1515 windows

#### calculate mean & median pi/pop ####

#calculate mean pi
Aen_AHam_pi_100bp_mean <- mean(Aen_AHam_pi_100bp$avg_pi) #0.00095
Aen_CHam_pi_100bp_mean <- mean(Aen_CHam_pi_100bp$avg_pi) #0.00106

#calculate median pi
Aen_AHam_pi_100bp_median <- median(Aen_AHam_pi_100bp$avg_pi) #0.00000
...
boot_Aen_AHam_pi_100bp <- boot(data = Aen_AHam_pi_100bp$avg_pi, statistic = samp_mean, R = 1000) #1000 permutations of pi
  ...
  Aen_AHam_pi_100bp_95ci_normal <- Aen_AHam_pi_100bp_95ci$normal #0.00087, 0.00104
...
boot_Aen_CHam_pi_100bp <- boot(data = Aen_CHam_pi_100bp$avg_pi, statistic = samp_mean, R = 1000) #1000 permutations of pi
  ...
  Aen_CHam_pi_100bp_95ci_normal <- Aen_CHam_pi_100bp_95ci$normal #0.00097, 0.00115

## write out pi summary table ##
pi_mean <- as.data.frame(c(0.00095, 0.00106))
...
Aen_pi_boot_diff$pi_diff <- Aen_pi_boot_diff$Contemp_pi - Aen_pi_boot_diff$Hist_pi
  Aen_pi_boot_diff_CIs <- quantile(Aen_pi_boot_diff$pi_diff, c(0.025, 0.975)) #-0.00001, 0.00023
...
#for loop to randomly sample df 10 000x
for (i in 1:10000) { 
  permutation_AHam <- Aen_pi_100bp[sample(nrow(Aen_pi_100bp), 1514), ] #random subset = # Alb rows
  permutation_CHam <- Aen_pi_100bp[sample(nrow(Aen_pi_100bp), 1515), ] #random subset = # Contemp rows
...
## Calculate empirical p-value ##
#number of permutations with value greater than observed
pi_perm_greater <- as.numeric(nrow(subset(permutation_diff_pi, permutation_pi > 0.00011))) 
  (pi_perm_greater + 1)/10001 #0.0071
...
pi_null_plot <- ggplot(permutation_diff_pi, aes(x = permutation_pi)) + 
  ...
  geom_vline(aes(xintercept = 0), linewidth = 4, color = "black") +
  geom_vline(aes(xintercept = -0.00009), linewidth = 6, linetype = "dashed", color = "#bd8850") + #0.25 CI lower
  geom_vline(aes(xintercept = 0.00009), linewidth = 6, linetype = "dashed", color = "#bd8850") + #0.95 CI upper
  geom_vline(aes(xintercept = 0.00011), linewidth = 6, color = "#a25505") + #"real" mean diff
  annotate("text", x = -0.00025, y = 1900, label = "G", size = 40) + 
  scale_y_continuous(expand = c(0, 0)) + 
  scale_x_continuous(labels = scales::label_number()) + 
  xlab("Temporal Change in π") + ylab("Density") + 
  theme_classic() + 
...
#calculate mean and median delta  
mean(na.omit(pi_merge$delta)) #0.00010--> gaining diversity through time
median(na.omit(pi_merge$delta)) #0

#identify ones that became negative through time (lost diversity)
#poss selective sweeps
pot_outliers <- subset(pi_merge, delta < 0) #358 windows

#diff scatter plot
pi_diff_scatter_plot <- ggplot(data = pi_merge, aes(x = Alb_avg_pi, y = Contemp_avg_pi)) + 
  geom_point(size = 14, color = "#a25505", alpha = 0.5) + 
  geom_abline(slope = 1, intercept = 0, linewidth = 4, linetype = "dashed", color = "black") + 
...
#subsetting dataframe to windows with >100 genotyped sites
Aen_unrelated_pi_100bp <- subset(Aen_unrelated_pi, no_sites >= 100) #left with 3029 windows (~1514/time point)

#create dataframe for each pop
Aen_unrelated_AHam_pi_100bp <- subset(Aen_unrelated_pi_100bp, pop == "AHam") #1514 windows
Aen_unrelated_CHam_pi_100bp <- subset(Aen_unrelated_pi_100bp, pop == "CBat") #1515 windows

#### calculate mean & median pi/pop ####

#calculate mean pi
Aen_unrelated_AHam_pi_100bp_mean <- mean(Aen_unrelated_AHam_pi_100bp$avg_pi) #0.00095
Aen_unrelated_CHam_pi_100bp_mean <- mean(Aen_unrelated_CHam_pi_100bp$avg_pi) #0.00106

#calculate median pi
Aen_unrelated_AHam_pi_100bp_median <- median(Aen_unrelated_AHam_pi_100bp$avg_pi) #0.00000
Aen_unrelated_CHam_pi_100bp_median <- median(Aen_unrelated_CHam_pi_100bp$avg_pi) #0.00000

#### pi bootstrapping ####

#bootstrap for Aen Albatross
boot_Aen_unrelated_AHam_pi_100bp <- boot(data = Aen_unrelated_AHam_pi_100bp$avg_pi, statistic = samp_mean, R = 1000) #1000 permutations of pi
  Aen_unrelated_AHam_pi_100bp_95ci <- boot.ci(boot_Aen_unrelated_AHam_pi_100bp, conf = 0.95, type = "norm")
  Aen_unrelated_AHam_pi_100bp_95ci_normal <- Aen_unrelated_AHam_pi_100bp_95ci$normal #0.00086, 0.00104

#bootstrap for Aen Contemporary
boot_Aen_unrelated_CHam_pi_100bp <- boot(data = Aen_unrelated_CHam_pi_100bp$avg_pi, statistic = samp_mean, R = 1000) #1000 permutations of pi
  Aen_unrelated_CHam_pi_100bp_95ci <- boot.ci(boot_Aen_unrelated_CHam_pi_100bp, conf = 0.95, type = "norm")
  Aen_unrelated_CHam_pi_100bp_95ci_normal <- Aen_unrelated_CHam_pi_100bp_95ci$normal #0.00096, 0.00116

## write out pi summary table ##
pi_mean <- as.data.frame(c(0.00095, 0.00106))

#pi summary table
pi_ci <- rbind(Aen_unrelated_AHam_pi_100bp_95ci_normal, 
               Aen_unrelated_CHam_pi_100bp_95ci_normal) #combine df w/ci info for each pop into one dataframe
pi_sum <- cbind(pi_mean, pi_ci)
  colnames(pi_sum) <- c("mean", "ci", "2.5_per", "97.5_per")
  pi_sum$Pop <- c("Hist", "Contemp")
...
#write out data
#write.csv(pi_sum, "Data/Aen_Ham/Aen_unrelated_pi_cis.csv")

## Calculate 95% for diff through time ##
Aen_unrelated_pi_boot_diff <- as.data.frame(boot_Aen_unrelated_AHam_pi_100bp$t)
  Aen_unrelated_pi_boot_diff$Contemp_pi <- boot_Aen_unrelated_CHam_pi_100bp$t
  colnames(Aen_unrelated_pi_boot_diff) <- c("Hist_pi", "Contemp_pi")

Aen_unrelated_pi_boot_diff$pi_diff <- Aen_unrelated_pi_boot_diff$Contemp_pi - Aen_unrelated_pi_boot_diff$Hist_pi
Aen_unrelated_pi_boot_diff_CIs <- quantile(Aen_unrelated_pi_boot_diff$pi_diff, 
                                           c(0.025, 0.975)) #-0.00003, 0.00024
...
#for loop to randomly sample df 10 000x
for (i in 1:10000) { 
  permutation_AHam <- Aen_unrelated_pi_100bp[sample(nrow(Aen_unrelated_pi_100bp), 1514), ] #random subset = # Alb rows
  permutation_CHam <- Aen_unrelated_pi_100bp[sample(nrow(Aen_unrelated_pi_100bp), 1515), ] #random subset = # Contemp rows
...
#write.csv(permutation_diff_pi, "Data/Aen_Ham/Aen_unrelated_permutation_pi.csv")

## Calculate empirical p-value ##
#number of permutations with value greater than observed
pi_perm_greater <- as.numeric(nrow(subset(permutation_diff_pi, permutation_pi > 0.00011))) 
(pi_perm_greater + 1)/10001 #0.00780
...
pi_plot <- ggplot(data = pi_sum, aes(x = Pop, y = mean, color = Pop, shape = Pop)) + 
  ...
  scale_color_manual(values = c("#a25505", "#e3ccb4")) +
  scale_shape_manual(values = c(19, 15)) +
  ggtitle("Aen unrelated pi 95% CIs") + theme_bw() + 
  theme(panel.border = element_rect(linewidth = 1), 
        plot.title = element_text(size = 30, color = "black", face = "bold"),
        axis.title = element_text(size = 28, color = "black"), 
        axis.ticks = element_line(color = "black", linewidth = 1), 
...
## merge Alb & Contemp ##
pi_Alb <- as.data.frame(Aen_unrelated_AHam_pi_100bp$chromosome)
  pi_Alb$pi <- Aen_unrelated_AHam_pi_100bp$avg_pi
  colnames(pi_Alb) <- c("chromosome", "avg_pi")
pi_Contemp <- as.data.frame(Aen_unrelated_CHam_pi_100bp$chromosome)
  pi_Contemp$pi <- Aen_unrelated_CHam_pi_100bp$avg_pi
  colnames(pi_Contemp) <- c("chromosome", "avg_pi")
...
#calculate mean and median delta  
mean(na.omit(pi_merge$delta)) #0.00011 --> gaining diversity through time
median(na.omit(pi_merge$delta)) #0

#identify ones that became negative through time (lost diversity)
#poss selective sweeps
pot_outliers <- subset(pi_merge, delta < 0) #356 windows

#diff scatter plot
pi_diff_scatter_plot <- ggplot(data = pi_merge, aes(x = Alb_avg_pi, y = Contemp_avg_pi)) + 
  geom_point(size = 14, color = "#a25505", alpha = 0.5) + 
  geom_abline(slope = 1, intercept = 0, linewidth = 4, linetype = "dashed", color = "black") + 
  ggtitle("Aen unrelated delta pi") + labs(y = "Contemporary", x = "Albatross") + 
  theme_bw() + 
  theme(panel.border = element_rect(linewidth = 1), 
        plot.title = element_text(size = 30, color = "black", face = "bold"),
        axis.title = element_text(size = 28, color = "black"), 
...
He_plot <- ggplot(data = all_data[all_data$metric == "He",], 
  ...
  annotate("text", x = 2.2, y = 0.012, label = "G. min: p < 0.05", size = 15, hjust = 0, color = "#1c3b0e") + 
...
Pi_plot <- ggplot(data = all_data[all_data$metric == "pi",], 
  ...
  annotate("text", x = 2.2, y = 0.0006, label = "A. end: p < 0.01", size = 15, hjust = 0, color = "#a25505") + 
  annotate("text", x = 2.2, y = 0.0004, label = "G. min: p > 0.05", size = 15, hjust = 0, color = "#1c3b0e") + 
  annotate("text", x = 2.2, y = 0.0002, label = "E. lat: p < 0.01", size = 15, hjust = 0, color = "#16537e") + 
  scale_color_manual(values = c("#e3ccb4", "#8aa9be", "#afc8a4", "#a25505", "#16537e", "#1c3b0e")) + 
  scale_shape_manual(values = c(19, 15)) +
  scale_x_discrete(labels = c("A.\nendrachtensis", "G.\nminuta", "E.\nlaterofenestra")) + 
  ylab("π mean") + 
...
Fis_plot <- ggplot(data = all_data[all_data$metric == "Fis",], 
  ...
  annotate("text", x = 0.6, y = 0.16, label = "D", size = 40) + 
  annotate("text", x = 2.2, y = 0.158, label = "A. end: p < 0.0001", size = 15, hjust = 0, color = "#a25505") + 
  annotate("text", x = 2.2, y = 0.148, label = "G. min: p > 0.05", size = 15, hjust = 0, color = "#1c3b0e") + 
  annotate("text", x = 2.2, y = 0.138, label = "E. lat: p > 0.05", size = 15, hjust = 0, color = "#16537e") + 
  scale_color_manual(values = c("#e3ccb4", "#8aa9be", "#afc8a4", "#a25505", "#16537e", "#1c3b0e")) + 
  scale_shape_manual(values = c(19, 15)) +
  scale_x_discrete(labels = c("A.\nendrachtensis", "G.\nminuta", "E.\nlaterofenestra")) + 
